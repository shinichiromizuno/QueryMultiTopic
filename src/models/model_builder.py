...
class Bert(nn.Module):
    ...
    def forward(self, x, segs, mask):
        ...
        encoded_layers = self.model(x, token_type_ids=segs, attention_mask =mask)
        # top_vec = encoded_layers[-1]
        top_vec = encoded_layers.last_hidden_state
        return top_vec
...
class BertBaseline(nn.Module):
    def __init__(self, temp_dir, load_pretrained_bert, bert_config):
        ...
        if(load_pretrained_bert):
            self.model = BertModel.from_pretrained('bert-base-uncased', cache_dir=temp_dir)
            # Below added for baseline
            self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
            self.tokenizer.add_tokens(['[L1]'])
...
class SummarizerBaseline(nn.Module):
    def __init__(self, args, device, load_pretrained_bert = False, bert_config = None):
        super(SummarizerBaseline, self).__init__()
        self.args = args
        self.device = device
...
class SummarizerProposed(nn.Module):
    def __init__(self, args, device, load_pretrained_bert = False, bert_config = None):
        super(SummarizerProposed, self).__init__()
        self.args = args
        self.device = device
        self.bert = Bert(args.temp_dir, load_pretrained_bert, bert_config)
        self.encoder = TransformerInterEncoder(self.bert.model.config.hidden_size, args.ff_size, args.heads,
